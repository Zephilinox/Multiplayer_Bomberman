...
void Menu::calculateButtonPositions()
{
    ...
    if (m_Buttons.size() % 2 == 0) //if the number of buttons is even
    {
        ...
        m_Buttons[LowerYButton].setPosition(sf::Vector2f(Constants::windowWidth/2 - m_Buttons[LowerYButton].getSize().x/2,
                                                         Constants::windowHeight/2 - m_Buttons[LowerYButton].getSize().y)); //top-left is 0, 0; so subtract all of y to place it above center screen

        unsigned int UpperYButton = (m_Buttons.size()/2.f);
        m_Buttons[UpperYButton].setPosition(sf::Vector2f(Constants::windowWidth/2 - m_Buttons[UpperYButton].getSize().x/2,
                                                         Constants::windowHeight/2)); //don't subtract size-y because the top is already in the center
    ...
    else //odd number of buttons
    {
        ...
        m_Buttons[CenterYButton].setPosition(sf::Vector2f(Constants::windowWidth/2 - m_Buttons[CenterYButton].getSize().x/2,
                                                          Constants::windowHeight/2 - m_Buttons[CenterYButton].getSize().y/2)); //top-left is 0, 0; so subtract half of y to place it in the center
